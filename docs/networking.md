...
This requires a `ServiceEntry` per service that declares the hostname as `MESH_INTERNAL`. These are **auto-generated by Kyverno** — a ClusterPolicy (`mesh-serviceentry-generation`) watches for VirtualServices targeting the `shared/private` gateway and creates STATIC ServiceEntries pointing to the gateway's ClusterIP (`platform.gatewayAddress`).
...
- **ownerReference on ServiceEntry.** Kyverno sets the VirtualService as owner, so deleting a VirtualService automatically garbage-collects its ServiceEntry.

ServiceEntries are fully automatic — any VirtualService referencing the `shared/private` gateway gets one. No chart changes needed when adding new apps.
